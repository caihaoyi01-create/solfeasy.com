...
The local preview remains at http://127.0.0.1:3000/. The Worker `solfeasy-com` is deployed with the `solfeasy.com` custom-domain route and the D1 database is connected. Real charges and live advertisements remain disabled because payment and AdSense credentials are not configured. The separate production verification server is stopped after checking. See `launch-notes.md` for configuration and operating instructions.
